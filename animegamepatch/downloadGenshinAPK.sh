downloadYesGenshin() {
    ...
    if [[ $dgenshininp = "1" ]]; then
        if [[ -f "/sdcard/Genshin_Impact_3.2-Termux.apk" ]]; then
            echo "${greenColorBold}Genshin_Impact_3.2-Termux.apk already exist in /sdcard !${whiteColor}"
            echo
            ...
            versionGenshin="3.2"
            if [[ -f "$HOME/Genshin_Impact_3.2-Termux.apk" ]]; then
                busybox rm "$HOME/Genshin_Impact_3.2-Termux.apk"
            fi
            wget https://github.com/Score-Inc/AnimeGamePatch/releases/download/3.2/Genshin_Impact_3.2-Termux.apk -q --show-progress
        fi
        ...
        if [[ -f "/sdcard/Genshin_Impact_3.0.apks" ]]; then
            ...
            wget https://github.com/Score-Inc/AnimeGamePatch/releases/download/3.0/AnimeGame-Termux-3.0-patched.apks -q --show-progress
        fi
        ...
        if [[ -f "/sdcard/Genshin_Impact_2.8.apks" ]]; then
            ...
            wget https://github.com/Score-Inc/AnimeGamePatch/releases/download/2.8/Genshin_Impact_2.8.apks -q --show-progress
        fi
        ...
        if [[ -f "/sdcard/Genshin.Impact.Cert.Patch_Sign.apk" ]]; then
            ...
            wget https://github.com/Score-Inc/AnimeGamePatch/releases/download/2.7/Genshin.Impact.Cert.Patch_Sign.apk -q --show-progress
        fi
    fi
    ...
    if [[ "$ifdownloadgenshinfailed" != 0 ]]; then
        ...
        sleep 0.5s
        if [[ -f "Genshin_Impact_3.2-Termux.apk" ]]; then
            busybox mv Genshin_Impact_3.2-Termux.apk /sdcard
        elif [[ -f "Genshin_Impact_2.8.apks" ]]; then
    ...
    if [[ -f "/sdcard/Genshin_Impact_3.2-Termux.apk" ]]; then
        echo -e "${greenColorBold}Success move to /sdcard with name Genshin_Impact_3.2-Termux.apk${whiteColor}"
        echo
        echo -n "Press enter for back to Menu!"
        read -r
        UIMenu
        return
    elif [[ -f "/sdcard/Genshin_Impact_2.8.apks" ]]; then
