extractMitm() {
    ...
    if [[ -f $mitmproxyPath ]]; then
        ...
        case $extractMitm_input in
            "y" | "Y" ) rm -rf $HOME/.ElaXan/mitmproxy; clear; showCowsay;;
            "n" | "N" ) UIMenu;;
    ...
    cd $HOME || exit 1
    run_Program() { wget https://github.com/ElaXan/AnimeGamePatch/releases/download/mitm/mitmproxy.tar.gz &> $HOME/zerr.log; errCode=$?; log "$errCode"; }
    run_Program & pid=$!
    ...
    if ! command -v pipx &> /dev/null; then
        run_Program() { python3 -m pip install --user pipx &> $HOME/zerr.log; errCode=$?; log "$errCode"; }
        run_Program & pid=$!
        spin "Install pipx${whiteColor}" "0" "Menu" "UIMenu"
        run_Program() { python3 -m pipx ensurepath &> $HOME/zerr.log; errCode=$?; log "$errCode"; }
        run_Program & pid=$!
