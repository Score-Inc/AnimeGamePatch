getCert() {
    ...
    if [[ $isRooted = true ]]; then
        if [[ -f $detectCertInstalled ]]; then
            ...
            "y" | "Y")
                su -c rm $detectCertInstalled
                echo "${greenColorBold}Certficiate done deleted...${whiteColor}"
                sleep 1s
                clear
                whoMadeThis
                ;;
            "n" | 'N')
                echo "${greenColorBold}Cancel deleted cert by users!${whiteColor}"
                sleep 1s
                clear
                whoMadeThis
                ;;
    ...
    run_Program() {
        timeout --foreground 10s ./.local/bin/mitmdump --ssl-insecure &>$HOME/zerr.log &
        errCode=$?
        log "$errCode"
        sleep 3
    }
    ...
    run_Program() {
        ...
        errCode=$?
        cp $HOME/zerr.log /sdcard/mitm.cer
        log "$errCode"
        sleep 1s
    }
    run_Program &
